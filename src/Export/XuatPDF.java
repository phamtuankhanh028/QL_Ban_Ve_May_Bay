...
public class XuatPDF {
	...
	public void xuatPDFPhieuNhap(JTable table, int ma) {
		...
		try {
			...
			Paragraph pdfTitle = new Paragraph(new Phrase("THÔNG TIN HOÁ ĐƠN", fontData2));
			// set title
			...
			// xuống dòng
			HoaDonDTO hd = HoaDonDAO.getmahoadon(ma);
			System.out.println(hd);
			Chunk glue = new Chunk(new VerticalPositionMark());// Khoang trong giua hang

			Paragraph paraMaHoaDon = new Paragraph(new Phrase("Hoá Đơn: " + String.valueOf(hd.mahd), fontData));

			doc.add(paraMaHoaDon);
			doc.add(Chunk.NEWLINE);

			// viết 1 hàm getnhacungcap để lấy tên nhà cung cấp
			NhanVienDTO nv = HoaDonDAO.getnhanvien(hd.getManv());
			// tương tự với nhân viên
			Paragraph para1 = new Paragraph();
			para1.setFont(fontData);
			para1.add(String.valueOf("Nhân Viên: " +nv.getMa_nv() +" - "+ nv.getTen_nv()));
			...
			para3.add(String.valueOf("Ngày : " +java.time.LocalDate.now()));
			para3.add(glue);
			doc.add(para3);
			doc.add(Chunk.NEWLINE);
